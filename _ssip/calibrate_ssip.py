...
CALIBRATION_SET = [
    ...
    # --- Donors (α > 0) ---
    ("methanol",         "CO",                       2.12, 2.19),  # Driver & Hunter PCCP 2020
    ("ethanol",          "CCO",                      2.06, 2.19),
    ("1-propanol",       "CCCO",                     2.06, 2.19),
    ("water",            "O",                        2.81, 4.50),
    ("phenol",           "Oc1ccccc1",                2.79, 1.83),
    ("chloroform",       "ClC(Cl)Cl",                1.73, 0.00),
    # acetic_acid: α from Driver & Hunter 2020; β excluded (carboxylic c-factor
    # computed automatically from E_min after fitting all other acceptors)
    ("acetic_acid",      "CC(=O)O",                  2.00, float("nan")),
...
C_FACTORS = {
    ...
    "alcohol_O":       0.95,
    "aldehyde_C=O":    0.89,
    "carboxylic_C=O":  0.55,  # auto-refined from acetic_acid E_min in run_calibration()
    "ester_C=O":       0.96,
    "carbonate_C=O":   0.96,
    "nitro_O":         0.80,
    "sulfoxide_O":     0.99,
...
def run_calibration(surface_dir: Path, output_path: Path):
    ...
    # Auto-compute carboxylic_C=O c-factor from acetic_acid E_min so that
    # β_pred(acetic acid) = 1.80 (Driver & Hunter PCCP 2020).
    c_factors = dict(C_FACTORS)
    target_beta_acetic = 1.80
    if "acetic_acid" in emax_emin:
        E_min_aa = emax_emin["acetic_acid"][1]
        raw_beta = _beta_model(E_min_aa, b0, b1)
        if raw_beta > 0:
            c_carboxylic = target_beta_acetic / raw_beta
            c_factors["carboxylic_C=O"] = round(c_carboxylic, 4)
            print(f"  carboxylic_C=O c-factor computed: {c_carboxylic:.4f} "
                  f"(raw β={raw_beta:.3f} → target={target_beta_acetic})")
